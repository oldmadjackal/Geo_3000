...
/*                      Выбор цвета по палитре                       */
    ...
    COLORREF Task_Area_color_choice(COLORREF color)  
{
   RSS_Kernel *Module ;
          int  coef ;
          int  row ;
...
/*--------------------------------------------- Пропись обработчиков */
   ...
   for(i=0 ; i<_CEIL_CNT ; i++)  
              Palette[i].choice_proc=Task_Area_set_color ;
     
/*---------------------------------------- Создание диалогового окна */

                      Color=color ;
                      Module=GetTaskAreaEntry() ;

    DialogBoxIndirect(GetModuleHandle(NULL),
			(LPCDLGTEMPLATE)Module->Resource("IDD_COLOR_CHOICE", RT_DIALOG),
			   GetActiveWindow(), Task_Area_color_dialog) ;
...
/*	   Обработчик сообщений диалогового окна VIEW	             */	
    ...
    BOOL CALLBACK  Task_Area_color_dialog(  HWND  hDlg,     UINT  Msg, 
...
/*                          Установка цвета                          */
 ...
 COLORREF  Task_Area_set_color(COLORREF color)
