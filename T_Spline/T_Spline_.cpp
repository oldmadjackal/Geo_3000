...
/*		     Общие члены класса             		    */
    ...
    struct RSS_Module_Spline_instr *RSS_Module_Spline::mInstrList ;
...

{
  ...
  int  RSS_Module_Spline::vExecuteCmd(const char *cmd, void **iface)
...
/*		      Реализация инструкции HELP                    */
  ...
  int  RSS_Module_Spline::cHelp(char *cmd, void **iface)
...
/*		      Реализация инструкции CONFIG                  */
  ...
  int  RSS_Module_Spline::cConfig(char *cmd, void **iface)
...
/*								    */
/*         DRIVE <Имя траектории>                                   */

  int  RSS_Module_Spline::cDrive(char *cmd, void **iface)
...
/*         GLIDE <Имя траектории>                                   */
  ...
  int  RSS_Module_Spline::cGlide(char *cmd, void **iface)
...
#define     CLEAR_RESULT()       if(iface!=NULL)   ((char *)*iface)[0]=0 ;
#define      FAIL_RESULT(error)  if(iface!=NULL && ((char *)*iface)[0]==0)  sprintf((char *)(*iface), "ERROR:%s", error) 
#define      GOOD_RESULT(text )  if(iface!=NULL && ((char *)*iface)[0]==0)  sprintf((char *)(*iface), "DONE:%s", text) 
...
/*- - - - - - - - - - - - - - - - - - -  Выделение ключей управления */
     ...
     if(*cmd=='/') {
                ...
                if(end==NULL) {
                      ...
                      FAIL_RESULT("Spline.Glide:Некорректный формат команды") ;
                                       return(-1) ;
                              }
                  *end=0 ;
                ...
                if(strchr(cmd, 'q')!=NULL ||
                   strchr(cmd, 'Q')!=NULL   )        quiet_flag=1 ;
                                ...
                                entry=strchr(cmd, 'z') ;
                if(entry==NULL) entry=strchr(cmd, 'Z') ;
                if(entry!=NULL)
                 if(isdigit(entry[1]))  mDebug_level=entry[1]-'0' ;
                 else                   mDebug_level=  1 ;
...
/*---------------------------------------- Контроль имени траектории */
    ...
    if(traj_name==NULL) {                                           /* Если имя не задано... */
                  ...
                  FAIL_RESULT("Не задано имя траектории.\n"
                              "Например: INDICATE <Имя_траектории> ...") ;
                                     return(-1) ;
                        }
...
/*------------------------- Извлечение описаний степеней подвижности */
     ...
     if(mDrives_cnt<=0) {
             SEND_ERROR("Объект не имеет степеней подвижности") ;
            FAIL_RESULT("Объект не имеет степеней подвижности") ;
                                return(-1) ;
                        }
/*------------------------------------ Контрольный прогон траектории */
     ...
     if(status) {
       if(iface==NULL)  SEND_ERROR("Базовая траектория непроходима") ;
                       FAIL_RESULT("Базовая траектория непроходима") ;
                                return(-1) ;
                }
...
#define  C     mDrives[i].contexts
    ...
    for(i=0 ; i<mDrives_cnt ; i++) {                                /* CIRCLE.1 - Перебираем степени подвижности */
       ...
       if(mDrives[i].contexts==NULL) {                              /* Если пустой список контекстов */
            ...
            FAIL_RESULT("У объекта не заданы параметры приводов.\n"
                        "Используйте команду SPLINE DRIVE <ИМЯ ОБЪЕКТА>.") ;
                                         return(-1) ;
                                     }
        ...
        if(data->v_max<=0. ||
           data->a_max<=0.   ) {
             SEND_ERROR("У объекта не заданы максимальные скорости/ускорения приводов.\n"
                        "Используйте команду SPLINE DRIVE <ИМЯ ОБЪЕКТА>.") ;
            FAIL_RESULT("У объекта не заданы максимальные скорости/ускорения приводов.\n"
                        "Используйте команду SPLINE DRIVE <ИМЯ ОБЪЕКТА>.") ;
                                         return(-1) ;
                               }
...
/*- - - - - - - - - - - - - - - Создание/извлечение контекста модуля */
       ...
       if(context==NULL) {                                          /* Если не удалось разместить... */
            SEND_ERROR("Недостаточно памяти для списка контекстов") ;
           FAIL_RESULT("Недостаточно памяти для списка контекстов") ;
                               return(-1) ;
                         }
...
/*- - - - - - - - - - - - - - - - -  Регистрация нового блока данных */
       ...
       if(data==NULL) {                                             /* Если не удалось разместить... */
            SEND_ERROR("Недостаточно памяти для контекста модуля") ;
           FAIL_RESULT("Недостаточно памяти для контекста модуля") ;
                           return(-1) ;
                      }
                             }
    ...
    if(W->Path_vectors==1) {
       ...
       for(i=0 ; i<W->Path_vectors-1 ; i+=2) {                      /* Формируем данные таймеринга */
           if(i==0)  sprintf(tmp, "%.4lf,", segment_time[i]+segment_time[i+1]/2.) ;
                 ...
                 GOOD_RESULT(text) ;                                /* Выдаем результат по интерфейсу межмодульной связи */
...
/*-------------------------------------------------------------------*/
                   ...
                   iDebug("Complete", NULL, mDebug_level) ;
...
/*------------------------------------------- Копирование траектории */
                           ...
                           sprintf(cmd, "Path trace %s", 
                                           path_object->Name) ;
    this->mModulePath->vExecuteCmd(cmd, (void **)&result) ;

/*---------------------------------------------------- Анализ ответа */

   if(stricmp(result, "DONE"))  return(-1) ;
...
/*- - - - - - - - - - - - - - - - - - - - - -  - Опр.режима движения */
       ...
       if(K<1.) {                                                   /* Рассчет закритического режима */
         ...
         if(t_cr<0.)  t_cr=0. ;
                      ...
                      Vo  =fabs(v1[i]) ;
                      t   =(-Vo+sqrt(Vo*Vo+Amax[i]*L[i]))/Amax[i] ;
                      t_cr=  t ;
                }
...
/*- - - - - - - - - - - - - - Контроль предельной начальной скорости */
     do {
          ...
          if(Vo<fabs(v1[i])) {                                      /* Если начальная скорость слишком велика - */
                                 Vo_correction=v1[i]/Vo ;           /*   - уменьшаем ее до допустимой           */ 
...
/*---------------------------- Рассчет по критерию КОНЕЧНОЙ СКОРОСТИ */
   ...
   if(regime==_V_CORRECTION) {
             ...
             SEND_ERROR("Коррекция по конечной скорости для участка поворота не РАБОТАЕТ.") ;
                                  return(-1) ;
                             }
/*----------------------- Определение предельно-допустимых скоростей */
...
/*------------------------- Определение предельной выходной скорости */
   ...
   for(i_base=0, i=0 ; i<mDrives_cnt ; i++) {                       /* Определение опорной степени */
       ...
       if(fabs(K)>1.)  i_base=i ;                                   /* В качастве опоной выбираем степень      */
                                                                    /*  с наибольшим коэф.превышения скорости */
                                            }
...
/*- - - - - - - - - - - - - - - - - - - -  Останавливающиеся степени */
            ...
            if(Vo_max<Vo) {                                         /* Если начальная скорость слишком велика - */
                                 Vo_correction=Vo_max/Vo ;          /*   - уменьшаем ее до допустимой           */ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - Степени с разгоном */
         else if(Vk*Vo>0. && fabs(Vk)>fabs(Vo)) {
...
/*- - - - - - - - - - - - - - - - Степени с сонаправленной скоростью */
         else if(Vk*Vo>0.) {
                                     L =fabs(L) ;
                                   sgn =Vk>0.?1.:-1. ;
                                     Vo=fabs(Vo) ;
...
/*---------------------------- Проверка сходимости с конечной точкой */
   ...
   for(i=0 ; i<mDrives_cnt ; i++) {
       ...
       if(fabs(p[i]-start[i+mDrives_cnt])>mAccuracy[i]) {
                   sprintf(tmp, "SPLINE.iTraceSegment@"
       ...
       if(fabs( v[i]-v0[i+mDrives_cnt])>
          ...
          fabs(v0[i]-v0[i+mDrives_cnt])>1.e-10 ) {
                   sprintf(tmp, "SPLINE.iTraceSegment@"
