...
/*		     Общие члены класса             		    */
    ...
    struct RSS_Module_Fast_instr *RSS_Module_Fast::mInstrList ;
...

{
  ...
  int  RSS_Module_Fast::vExecuteCmd(const char *cmd, void **iface)
...
/*		      Реализация инструкции HELP                    */
  ...
  int  RSS_Module_Fast::cHelp(char *cmd, void **iface)
...
/*		      Реализация инструкции CONFIG                  */
  ...
  int  RSS_Module_Fast::cConfig(char *cmd, void **iface)
...
/*		      Реализация инструкции OPTIMIZE                */
  ...
  int  RSS_Module_Fast::cOptimize(char *cmd, void **iface)
...
/*-------------------------------------- Обсчет начальной траектории */
     ...
     do {
              iExecute(line_check_cmd, result) ;                    /* Проверка проходимости траектории */
          if(!memicmp(result, "FATAL", strlen("FATAL")))  break ;
          if(!memicmp(result, "ERROR", strlen("ERROR")))  break ;

              iExecute(time_calc_cmd, result) ;                     /* Опр.времени прохождения траектории */
          if(!memicmp(result, "FATAL", strlen("FATAL")))  break ;
          if(!memicmp(result, "ERROR", strlen("ERROR")))  break ;

                 path_time=strtod(result+5, &end) ;
                           strcpy(result_best, result) ;
...
/*-------------------------------------- Инициализация оптимизаторов */
         ...
         iMethodSimplif(&best_object, NULL, NULL, NULL, result, &path_time) ;
         iMethodCut    (&best_object, NULL, NULL, NULL, result, &path_time) ;
     ...
     if(method_use==0)
       if(this->mSimplification_opt)
          status=iMethodSimplif(&best_object,
                                &work_object, &n, &new_best, result, &path_time) ;
       else                           method_use++ ;
...
/*- - - - - - - - - - - - - - - - -  Управление методами оптимизации */
     if(status) {                                                   /* Если очередной метод завершил оптимизацию... */
            ...
            strcpy(result, result_best) ;                           /* Восстанавливаем "базовый" результат */
        if(memicmp(result, "DONE", strlen("DONE")))                 /* Опр.текущего "базового" времени */
                            path_time=-1. ;
        else                path_time=strtod(result+5, &end) ;           

     if(method_use==0)  iMethodSimplif(&best_object, NULL, NULL, NULL, result, &path_time) ;
//   if(method_use==1)  iMethodSimplif(&best_object, NULL, NULL, NULL, result, &path_time) ;
     if(method_use==2)  iMethodCut    (&best_object, NULL, NULL, NULL, result, &path_time) ;
...
/*- - - - - - - - - - - - - - - - - - - -  Обсчет рабочей траектории */
     ...
     do {
       ...
       if(!memicmp(result, "FATAL", strlen("FATAL")))  break ;      /* При системных ошибках прекращаем оптимизацию */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

      } while(1) ;

//         if(!quiet_flag)                                          /*  Если не режим "тешины" - */
                SEND_ERROR("Оптимизация завершена.") ;
...
/*- - - - - - - - - - - - - - - - - Разборка списка времен сегментов */
       if(!memicmp(result, "FATAL", strlen("FATAL")))  return(-1) ;
       if(!memicmp(result, "ERROR", strlen("ERROR")))  return(-1) ;

                 best_time=strtod(result+5, &end) ;                 /* Извлекаем лучшее время */
...
/*	               Выполнение команды                           */
  ...
  void  RSS_Module_Fast::iExecute(char *execute, char *result)
...
/*------------------------------ Опрос командных процессоров модулей */
    ...
    for(i=0 ; i<this->kernel->modules_cnt ; i++) {
      ...
      if(result!=NULL)  status=this->kernel->modules[i].entry->vExecuteCmd(command, (void **)&result) ;
