...
/*------------------------- Описание класса управления общим модулем */
  ...
  class RSS_Module_Main : public RSS_Kernel {
    ...
    public:
                     ...
                     int  cRobotStep    (char *) ;              /* Инструкция ROBOTSTEP */
...
/* Geo_2D.cpp */
      int  G2D_system           (void) ;                  /* Обработка системных сообщений */
      int  G2D_error            (char *, int) ;           /* Обработка сообщений об ошибках */
      int  G2D_command_start    (char *) ;                /* Запуск команд */
      int  G2D_command_processor(char *) ;                /* Процессор команд */
      int  G2D_command_read     (char **) ;               /* Обработчик командного файла */
      int  G2D_command_write    (char *) ;                /* Запись командного файла */
     void  G2D_command_normalise(char *) ;                /* Нормализация командной строки */
...
/* G2D_monitor.cpp */
      int  G2D_threads(char *, char *) ;      /* Мониторинг рабочих потоков */ 
      int  G2D_modules(char *) ;              /* Отображение списка присоединенных модулей */ 
