...
/*------------------------------------ Обработчики элементов диалога */
  ...
  static union WndProc_par  Tmp_WndProc ;
   ...
   LRESULT CALLBACK  G2D_Cmd_WndProc(HWND, UINT, WPARAM, LPARAM) ;

/*------------------------------------ Процедуры обработки сообщений */

   LRESULT CALLBACK  G2D_window_processor(HWND, UINT, WPARAM, LPARAM) ;
      BOOL CALLBACK  G2D_main_dialog     (HWND, UINT, WPARAM, LPARAM) ;

      UINT CALLBACK  iG2D_read_HookProc  (HWND, UINT, WPARAM, LPARAM) ;
...
/*---------------------------------------- Подгрузка рабочих модулей */
	    ...
	    Kernel.SetKernel(NULL, hInstance) ;
         ...
         if(Kernel.modules_cnt==0)
                      G2D_error("No active modules loaded", MB_ICONERROR) ;
...
/*------------------------------------------- Подготовка компонентов */
                      ...
                      G2D_show_init() ;                             /* Инициализация системы отображения */

      RSS_Kernel::display.GetList        =G2D_show_getlist ;        /* Пропись функций интерфейса */
      RSS_Kernel::display.ReleaseList    =G2D_show_releaselist ;        
      RSS_Kernel::display.SetFirstContext=G2D_first_context ;
      RSS_Kernel::display.SetNextContext =G2D_next_context ;
      RSS_Kernel::display.ShowContext    =G2D_show_context ;
      RSS_Kernel::display.GetContextPar  =G2D_get_context ;

/*---------------------------------------------------- Инициализация */

                      hInst=hInstance ;

      sprintf(ClassName, "G2D_Frame_%d", _getpid()) ;

/*------------------------------- Регистрация класса первичного окна */

	FrameWindow.lpszClassName= ClassName ;
	FrameWindow.hInstance    = hInstance ;
	FrameWindow.lpfnWndProc  = G2D_window_processor ;
    ...
    if(!RegisterClass(&FrameWindow)) {
          sprintf(text, "G2D_Frame register error %d", GetLastError()) ;
        G2D_error(text, MB_ICONERROR) ;
	                                return(-1) ;
				     }
/*----------------------------------------- Создание первичного окна */

    hFrameWindow=CreateWindow(ClassName, "Geo_2D", 
...
/*		Обработчик сообщений "рамочного" окна		     */
 ...
 LRESULT CALLBACK  G2D_window_processor(  HWND  hWnd,     UINT  Msg,
...
/*- - - - - - - - - - - - - - - - -  Перехват обработчиков сообщений */
       Tmp_WndProc.call=G2D_Cmd_WndProc ;
...
/*- - - - - - - - - - - - - - - - - - - - - - -  Сообщение об ошибке */
        if(wParam==_USER_ERROR_MESSAGE) {
                                     ...
                                     G2D_error((char *)lParam, MB_ICONERROR) ;
                			      return(FALSE) ;
                                        }
/*- - - - - - - - - - - - - - - - - - - - - - -  Сообщение об ошибке */
...
/*	          Обработка сообщений об ошибках                     */    
   ...
   int  G2D_error(char *message, int  icon)
...
/*	          Новыe обработчики элементов                        */    
  ...
  LRESULT CALLBACK  G2D_Cmd_WndProc(  HWND  hWnd,     UINT  Msg,
	                            WPARAM  wParam, LPARAM  lParam) 
{
 LRESULT  result ;
    BYTE  key_status[256] ;
...
/*                       Запуск команд                              */
   ...
   int  G2D_command_start(char *cmd)
...
/*------------------------------------------------------- Подготовка */
              ...
              G2D_threads("ADD", command) ;                         /* Регистрируем рабочий поток */

/*------------------------------------------------ Выпонение команды */

                G2D_command_processor(command) ;

/*------------------------------------------------------- Завершение */

     if(WaitCommandEnd)  SetEvent(hCommandEnd) ;

              G2D_threads("DELETE", NULL) ;                         /* Исключаем рабочий поток */
...
/*                    Процессор команд                              */
   ...
   int  G2D_command_processor(char *command)
...
typedef  struct {
  ...
  static  G2D_commands *commands_pool[100] ;   /* Список наборов команд */
...
/*-------------------------------------------- Цикл командных файлов */
  ...
  do {                                                              /* CIRCLE.0 */
        ...
        if(file_rd_flag) {                                          /* Считывание команды из файла */
                            status=G2D_command_read(&command) ;
                ...
                G2D_command_normalise(command) ;                    /* Нормализация команды */

        if(file_wr_flag) {                                          /* Запись команды в файл */
                            G2D_command_write(command) ;
                         }
...
#define  CP  commands_pool
        ...
        for(i=0 ; CP[i] ; i++)
          ...
          if(CP[i]==NULL) {
                CP[i]=(G2D_commands *)calloc(1, sizeof(*CP[i])) ;
                          }
   ...
   for(next=command ; next!=NULL ; command=next+1) {                /* CIRCLE.1 */
     ...
     for(next=command ; *next ; next++) {
      ...
      if( next!=NULL) *next=  0 ;
                ...
                G2D_command_normalise(cmd_buff) ;
...
/*                 Обработчик командного файла                      */
   ...
   int  G2D_command_read(char **command)
...
/*--------------------------------- Предварительная обработка строки */
                ...
                G2D_command_normalise(buff) ;                       /* Нормализация строки */
...
/*--------------------------------------------- Команда - Примечание */
            ...
            MessageBox(NULL, buff+strlen("#remark "), "Geo_2D",
...
/*                   Запись командного файла                        */
   ...
   int  G2D_command_write(char *command)
...
/*                 Нормализация командной строки                    */
  ...
  void  G2D_command_normalise(char *command)
...
/*                            Список команд                         */
 ...
 { "robotstep", "rstep", "# ROBOTSTEP - задание шагов изменения степеней подвижности",
                         " ROBOTSTEP <Угловой шаг> [<Линейный шаг>]\n"
                         " ROBOTSTEP/J <Угловой шаг>\n"
                         " ROBOTSTEP/L <Линейный шаг>\n",
                         &RSS_Module_Main::cRobotStep },
...

{
   char  text[128] ;
        ...
        if(!stricmp(name, "ROBOT_STEP_JOINT")) {
                        sprintf(value, "%20.10lf", __robot_step_joint) ;
                                               }
   else if(!stricmp(name, "ROBOT_STEP_LINK" )) {
                        sprintf(value, "%20.10lf", __robot_step_link) ;
                                               }
   else                                        {
...
/*-------------------------------------- Получение параметров камеры */
       ...
       status=G2D_look("Main", "Get", &look_x, &look_y, &look_z,
...
/*-------------------------------------- Установка параметров камеры */
       ...
       status=G2D_look("Main", "Set", &look_x, &look_y, &look_z,
                                         NULL,    NULL,    NULL ) ;
    if(status) {
                    return(-1) ;
               }
...
/*-------------------------------------- Получение параметров камеры */
       ...
       status=G2D_look("Main", "Get",    NULL,    NULL,    NULL, 
                                      &look_a, &look_e, &look_r ) ;
    if(status) {
                   SEND_ERROR("Неизвестное окно") ;
                      return(-1) ;
...
/*-------------------------------------- Установка параметров камеры */
       ...
       status=G2D_look("Main", "Set",    NULL,    NULL,    NULL, 
                                      &look_a, &look_e, &look_r ) ;
    if(status) {
                    return(-1) ;
               }
...
/*------------------------------------------------ Извлечение данных */
    ...
    if(status) {
              ...
              G2D_zoom  ("Main", "Get", &zoom) ;
              G2D_lookat("Main", "Get",  look_at) ;
...
/********************************************************************/
/*								    */
/*                Реализация инструкции ROBOTSTEP                   */
/*								    */
/*      ROBOTSTEP <Угловой шаг> [<Линейный шаг>]		    */
/*      ROBOTSTEP/J <Угловой шаг>               		    */
/*      ROBOTSTEP/L <Линейный шаг>		                    */

  int  RSS_Module_Main::cRobotStep(char *cmd) 
...
/*------------------------------------------------ Разборка диапазона */
    ...
    for(i=0 ; pars[i]!=NULL && *pars[i]!=0 ; i++) {

                value=atof(pars[i]) ;

      if(i==0 || joint_link_flag=='H') __robot_step_joint=value ;
      if(i==1 || joint_link_flag=='L') __robot_step_link =value ;
                                                  }
/*-------------------------------------------------------------------*/
    ...
    if(path[0]==0) {
	   ...
	   memset(&file_choice, 0, sizeof(file_choice))  ;
		   ...
		   file_choice.Flags       = OFN_ENABLEHOOK   |
                                             OFN_FILEMUSTEXIST ;
		   file_choice.lpfnHook    = iG2D_read_HookProc ;
...
/*-------------------------------------------------------------------*/
  ...
  UINT CALLBACK  iG2D_read_HookProc(  HWND hDlg,     UINT uiMsg, 
                                    WPARAM wParam, LPARAM lParam )
{
